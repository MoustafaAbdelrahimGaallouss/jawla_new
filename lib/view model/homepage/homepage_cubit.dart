...
class HomePageCubit extends Cubit<AppState> {
  ...
  goToProgramDetails2(int? id) {
    Get.toNamed(AppRoutes().programDetails, arguments: {"id": id});
  }

  goToSpecialServices() {
    Get.toNamed(
      AppRoutes().specialServices,
      arguments: {"serviceIndex": index},
    );
  }
    ...
    response.fold(
      (l) {
        if (l.type == StatusType.internetFailure ||
          ...
          emit(
            l.type == StatusType.internetFailure
                ? InternetError()
                : ServerError(),
          );
        }
